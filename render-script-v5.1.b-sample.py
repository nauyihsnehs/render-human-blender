...
DEFAULT_CONFIG = {
    ...
    "N_cameras_per_yaw": 1,
    "N_lights_per_camera": 3,

    # Fixed light directions for test-set sampling (theta/phi in degrees).
    # light_id will map to index in this list: 0..2; no-light uses 999.
    "fixed_lights": [
        {"theta_deg": 45.0, "phi_deg": 0.0, "name": "left"},
        {"theta_deg": -45.0, "phi_deg": 0.0, "name": "right"},
        {"theta_deg": 0.0, "phi_deg": 10.0, "name": "front_high"},
    ],
...
def build_scene_bvh_cache(scene_meshes, dg):
    cache = []
    for o in scene_meshes:
        bvh, obj_eval = bvh_from_object(o, dg)
        cache.append({"obj": o, "bvh": bvh, "obj_eval": obj_eval})
    return cache


def clear_scene_bvh_cache(scene_bvh_cache):
    for entry in scene_bvh_cache or []:
        try:
            entry["obj_eval"].to_mesh_clear()
        except Exception:
            pass


def min_distance_to_scene_meshes(point_w: Vector, scene_bvh_cache):
    best = 1e18
    for entry in scene_bvh_cache:
        bvh = entry["bvh"]
...
def sample_camera(cfg, rng, people_obj, people_origin_w, scene_bvh_cache, exclude_names):
    ...
    for st in range(int(cfg["cam_max_attempts"])):
        ...
        if ray_occluded(scene, dg, cam_pos, origin_pos, exclude_names=exclude_names):
            ...
            continue
        if min_distance_to_scene_meshes(cam_pos, scene_bvh_cache) < min_clear_bu:
...
def sample_fixed_light(cfg, rng, people_origin_w, cam_pos, scene_bvh_cache, exclude_names, theta_deg, phi_deg):
    ...
    if (light_pos - cam_pos).length < min_cam_sep_bu:
        print('Failed fixed light sample: too close to camera.')
        return None
    ...
    if ray_occluded(scene, dg, light_pos, people_origin_w, exclude_names=exclude_names):
        print('Failed fixed light sample: occluded to people.')
        return None
    if min_distance_to_scene_meshes(light_pos, scene_bvh_cache) < min_clear_bu:
        print('Failed fixed light sample: too close to scene mesh.')
        return None
...
def main(cfg):
    ...
    fixed_lights = cfg.get("fixed_lights") or []
    ...
    if not isinstance(fixed_lights, list) or len(fixed_lights) < n_lgt:
        raise RuntimeError("fixed_lights must provide at least N_lights_per_camera entries.")
    ...
    dg = depsgraph()
    scene_bvh_cache = build_scene_bvh_cache(scene_meshes, dg)
        ...
        try:
            ...
            for yaw_local in range(n_yaw):
                ...
                for cam_local in range(n_cam):
                    ...
                    rng_cam = new_rng()
                    cam_rec = sample_camera(cfg, rng_cam, people_obj, people_origin, scene_bvh_cache, exclude_now)
                    if cam_rec is None:
                    ...
                    for light_local in range(n_lgt):
                        ...
                        rng_lgt = new_rng()
                        light_cfg = fixed_lights[light_local]
                        light_rec = sample_fixed_light(
                            cfg,
                            rng_lgt,
                            people_origin,
                            cam_pos,
                            scene_bvh_cache,
                            exclude_now,
                            light_cfg.get("theta_deg"),
                            light_cfg.get("phi_deg"),
                        )
                        if light_rec is None:
                            print(
                                f"[WARN] Skipping light_id={light_id} at camera_id={camera_id} due to fixed light collision/occlusion.",
                                flush=True,
                            )
                            continue
                        successful_lights += 1
                    ...
                    if successful_lights == 0:
                        raise _PositionRetry(f"all fixed lights failed at camera_id={camera_id}")
    ...
    clear_scene_bvh_cache(scene_bvh_cache)
    try:
        floor_eval.to_mesh_clear()
    except Exception:
        pass
