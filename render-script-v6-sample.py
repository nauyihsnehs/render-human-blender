...
DEFAULT_CONFIG = {
    ...
    # Scale factor for original lighting (scene lights, environment map, emission).
    # Applied to lit samples only; no-light samples keep original lighting.
    "original_lighting_scale": 1.0,
...
def sample_light(cfg, rng, people_origin_w, cam_pos, scene_bvh_cache, exclude_names):
...
def main(cfg):
    ...
    original_lighting_scale = clamp01(float(cfg.get("original_lighting_scale", 1.0)))
    ...
    meta = {
        ...
        "S_people": s,
        "rgb_format": _rgb_fmt,
        "original_lighting_scale": float(original_lighting_scale),
            ...
            for yaw_local in range(n_yaw):
                ...
                for cam_local in range(n_cam):
                    ...
                    for light_local in range(n_lgt):
                        ...
                        rng_lgt = new_rng()
                        light_rec = sample_light(cfg, rng_lgt, people_origin, cam_pos, scene_bvh_cache, exclude_now)
