...
def main(cfg):
    ...
    ensure_compositor_nodes(int(cfg.get("human_pass_index")), cfg.get("rgb_format", "PNG"))
    ...
    init_light_energy = float(light_obj.data.energy)
    ...
    while len(meta["shared_positions"]) < n_pos:
        ...
        def _rollback_abandoned_position():
            ...
            # Delete any output files already written for this (pos_id) id range
            cam_id_start = pos_id * n_yaw * n_cam
            cam_id_end = cam_id_start + (n_yaw * n_cam) - 1
            for cam_id in range(cam_id_start, cam_id_end + 1):
                for fp in glob.glob(os.path.join(run_dir, f"{cam_id:03d}_*")):
                    try:
                        os.remove(fp)
                    except Exception:
                        pass
        ...
        try:
            ...
            for yaw_local in range(n_yaw):
                ...
                for cam_local in range(n_cam):
                    ...
                    render_and_rename(run_dir, frame, camera_id, 999, write_data_once=True, write_mask_once=True)
                    ...
                    # Flush metadata after each rendered frame (robust to crashes)
                    atomic_write_json(meta_path, meta, indent=2)

                    light_obj.data.energy = init_light_energy
                    ...
                    for light_local in range(n_lgt):
                        ...
                        light_obj.data.size = float(light_rec.get("size_bu", light_rec["size_m"]))
                        c = light_rec["color"]
                        ...
                        render_and_rename(run_dir, frame, camera_id, light_id, write_data_once=False, write_mask_once=False)
